...
export default function LaporanPenelitianForm() {
  const { id } = useParams();
  const isEdit = id && id !== 'new';
  const navigate = useNavigate();
  const [loading, setLoading] = useState(isEdit);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ judul: '', penulis: '', deskripsi: '', file_url: '', tahun: '', status: 'draft' });
  const [file, setFile] = useState(null);

  useEffect(() => {
    if (isEdit) {
      api.get(`/laporan-penelitian/${id}`).then(res => {
        const d = res.data.data;
        setForm({ judul: d.judul || '', penulis: d.penulis || '', deskripsi: d.deskripsi || '', file_url: d.file_url || '', tahun: d.tahun || '', status: d.status || 'draft' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.put(`/laporan-penelitian/${id}`, payload);
      } else {
        await api.post('/laporan-penelitian', payload);
      }
      navigate('/publikasi/laporan-penelitian');
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">
        {isEdit ? 'Edit Laporan Penelitian' : 'Tambah Laporan Penelitian'}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/publikasi/laporan-penelitian')} className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg text-sm font-medium transition">
            Batal
          </button>
        </div>
      </form>
