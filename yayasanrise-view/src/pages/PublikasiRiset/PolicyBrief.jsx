  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(t('publikasiRiset.policyBrief.error') + err.message);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="relative min-h-screen w-full font-sans bg-slate-50">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="rise-main max-w-3xl">
          <h1 className="text-4xl font-serif text-gray-800 text-center mb-2">{t('publikasiRiset.policyBrief.title')}</h1>
          <p className="text-center text-gray-600 text-sm mb-8 font-light">
            {t('publikasiRiset.policyBrief.description')}
          </p>
          <div className="space-y-3 w-full">
            {loading && <p className="text-center text-gray-500">{t('publikasiRiset.policyBrief.loading')}</p>}
            ...
            {!loading && !error && items.map((item) => (
              ...
              >
                ...
                <span className="text-xs text-rise-green bg-emerald-50 px-2 py-1 rounded">
                  {item.file_url ? t('publikasiRiset.policyBrief.download') : t('publikasiRiset.policyBrief.soon')}
                </span>
              </a>
            ))}
          </div>
